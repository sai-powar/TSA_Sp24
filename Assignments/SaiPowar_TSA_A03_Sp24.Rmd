...
p1 <- ggplot(energy_data, aes(x=energy_date, y=energy_data[,2])) +
            ...
            #geom_smooth(aes(y=dt_biomass_linear),color="orange",method="lm")
p1
...
p3 <- ggplot(energy_data, aes(x=energy_date, y=energy_data[,4])) +
            ...
            geom_line(aes(y=dt_re_linear), col="darkgreen")
p3
...
The ACF and PACF plots haven't changed significantly, so there is still time dependence remaining in the series. 
...
The hydroelectric power consumption series definitely has a seasonal component. There seems to be a seasonal component in the other two series as well, but in shorter time frames not the entire series. But, it is not as prominent as it is in the hydroelectric series. 
