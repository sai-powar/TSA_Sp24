...
> Answer: By using nsdiff, I identified that D=1 because seasonal differencing is needed. By using the ACF and PACF plots of the twice differenced series, I identified that Q = 1 and P = 0 because P+Q cannot be more than 1. 
The residuals plot looks like a white noise series, with the mean centered around 0 and no particular trend. But, there are some significant coefficients in the ACF and PACF plots, so that needs  
...
natgas_trend_diff <- diff(ts_natgas,lag =1, differences=1)
...
plot_grid(
  ...
  autoplot(Acf(natgas_trend_diff, lag = 40, plot=FALSE),  
                  main = "Trend-Differenced"),
  autoplot(Acf(natgas_both_diff, lag = 40, plot=FALSE),  
...
plot_grid(
  ...
  autoplot(Pacf(natgas_seas_diff, lag = 40, plot=FALSE),  
                  main = "Seasonal-Differenced"),
  autoplot(Pacf(natgas_trend_diff, lag = 40, plot=FALSE),  
                  main = "Trend-Differenced"),
  autoplot(Pacf(natgas_both_diff, lag = 40, plot=FALSE),  
                  main = "Twice-Differenced"),
  nrow=2
)
...
plot_grid(
  autoplot(Acf(natgas_both_diff,lag.max=60,plot=FALSE),main="Twice-Differenced",ylim=c(-1,1)),
  autoplot(Pacf(natgas_both_diff,lag.max=60,plot=FALSE),main="Twice-Differenced",ylim=c(-1,1)),
  nrow=1
)
```
```{r}
#Creating SARIMA model
Model1 <- Arima(ts_natgas,order=c(1,1,0),seasonal=c(0,1,1),include.drift=FALSE)
print(Model1)
