...
#The ACF shows trend and seasonality. PACF shows that it is an autoregressive process. 
```
...
#random is centered at 0. random component is truly random, no wave-like pattern, no trend 
```
...
#just removed the seasonal componnent from the original series, has to take a decomposed object 
  

#Plotting orinal and deseasoned series
autoplot(deseasonal_residential_price)
#residential price series without the seasonal component 
...
#very high autoregressive component, so nothing has been done to the trend 
```
...
#accept the null hypothesis, there is a unit root 
...
#decay is gone. by differencing we eliminated the trend 
...
print(Model_111)
#small magnitude coefficient, small correlation components
#we want a low AIC, the lower the AIC the better is the model (including the negative sign)
...
#want the acf and pacf to be inside the blue line, the residual series doesn't have a pattern
