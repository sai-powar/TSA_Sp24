...
ggplot(inflow_data_new_yearly, aes(x=my_year, y=inflow_data_yearly)) +
            geom_line(color="blue") +
            geom_smooth(color="red",method="lm") 
#not seasonal because not equally spaced 
```
...
#S is positive -> increasing trend 
#p-value is <0.05 so series does have a trend 
#across the seasons there is an increasing trend
...
#there is a trend across the annual averages 
...
#reject null hypothesis - accept the hypothesis that we have a trend stationary
...
#if adf tells you there is a trend, then you do differencing 
```
