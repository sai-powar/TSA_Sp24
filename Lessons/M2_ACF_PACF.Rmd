...
#parse=change the object from chr to date or whatever else
...
```{r}
ts_inflow_data <- ts(inflow_data[,2],start = c(1931,1),frequency=12) 
#can also use name inflow_data[,"HP1"]
#start(year to start, month to start(Jan so 1))
ts_inflow_data
...
#because its time series, autoplot knows x is the time
#instead of geom_line, use autolayer
```
...
# ACF and PACF measure the temporal dependency of a stochastic process. Give us info about the auto-regressive component of the series. 
#auto-regressive = series has a temporal dependency, so we need to add auto-regressive component to our model 
# PACF tells us which lags we need to add to our model 

# autocorrelation function = acf, will give self-correlation within our time series
# acf needs to receive a time series object
# HP1_acf$acf to see the values
# acf tells you info about trend and the seasonal pattern 
# interpretation - there is high correlation between the first observation and the 12th observation (Jan is correlated to next Jan). There is positive and negative correlation after every 6 months
# anything that is outside the blue lines will be considered significant 
...
#lag.max = specify the number of lags
HP1_acf <- Acf(ts_inflow_data)
