...
#look at class recording for interpreting the s.e.
#fitted - estimate we get from the model
#residual - estimate - the original data
...
#want the residual to be white noise -> mean to be centered around 0, no serial correlation or time dependence between the errors so ACF and PACF should have insignificant coeff


#Now let's try ARIMA(0,1,1)
#0 means no AR component
...
#increase in order, increase in coefficient
#ar2 = coeff with respect to y(t-1)
#drift is the intercept term, and should be close to 0 
#between the models, the middle terms has to be 1 because we need the differencing 
...
<<<<<<< HEAD:Lessons/SP_M5_ARIMAmodels_ClassFeb27.Rmd
Model_autofit <- auto.arima(deseasonal_residential_price, max.D=0,max.P=0,max.Q=0)
#max.D=0,max.P=0,max.Q=0 -> don't fit a seasonal model because we know we don't have seasonality in our data 
#lowercase max.d, p, q will be for the non-seasonal parts of our dataset 
# ar1 = phi, ma1 = theta 
#ar1 is the relationship between y(t) - y(t-1) and ma1 is the relationship between a(t) and a(t-1)

=======
Model_autofit <- auto.arima(deseasonal_residential_price,max.D=0,max.P=0,max.Q=0)
print(Model_autofit)
>>>>>>> 24e360d003f485194c259ab9e60f55b59c2df0d0:Lessons/M5_ARIMAmodels_ClassFeb27.Rmd
...
#You will get an error! Because this is a non-stationary series 
...
=======
  autolayer(Model_212$fitted,series="ARIMA(2,1,2)")+
  autolayer(Model_011$fitted,series="ARIMA(0,1,1)")



>>>>>>> 24e360d003f485194c259ab9e60f55b59c2df0d0:Lessons/M5_ARIMAmodels_ClassFeb27.Rmd

#all lead to similar fitted values. 
```
