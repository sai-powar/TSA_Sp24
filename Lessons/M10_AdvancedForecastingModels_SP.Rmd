...
unlink(temp)
#remove the temp file from environment 
...
act_power_hourly <- act_power_all %>% 
  ...
  summarise( hourly_mean_active_power = mean(Global_active_power)) #60 rows replaced by the mean 
#if we had NAs after aggregation then we would run ts_clean 
...
<<<<<<< HEAD:Lessons/M10_AdvancedForecastingModels_SP.Rmd
* **data** numeric vector, matrix or data frame  
* **seasonal.periods** vector with the seasonal periods  
* **ts.frequency** seasonal period that should be used as frequency of the underlying ts object. The default value is max(seasonal.periods)  [need to specify mainly when we know the source of the seasonality is not from the largest frequency]
...
-   **start** starting date/time of the time series. Now instead of only using start=`c(year, month)` we will use for example `start=c(year,month,day)`
>>>>>>> e60e521a3083942dec9fa90585d5c0036c5732cf:Lessons/M10_AdvancedForecastingModels.Rmd
...
<<<<<<< HEAD:Lessons/M10_AdvancedForecastingModels_SP.Rmd
                            seasonal.periods =c(24,168,8766), #vector of all the seasonal frequencies we need to account for
                            start=c(2006,12,16)) #need to specify hour if the data does not start at the first hour (rarely happens)
=======
...
ARIMA_Four_fit <- auto.arima(ts_act_power_daily_train, 
                             ...
                             xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,12)) #K represents the seasonality regressors, rewatch class recording for this part, 12 = maximum number of terms to model the seasonality of the dataset (can be less/more)
                             )
#K=c(2,12) -> 2=order depending on the seasonal periods, 12=modelling the yearly seasonality
#if it is daily, then we would have 24. Look at the 3-28 recording 
...
ARIMA_Four_for <- forecast(ARIMA_Four_fit,
                           xreg=fourier(ts_act_power_daily_train,
                                        K=c(2,12),
                                        h=365), #forecast 365 fourier terms
                           h=365 #forecast 365 terms for the arima model 
                           ) 
#no h just fitting the data, with h -> forecasting the data 
#Plot foresting results
...
[Confidence interval are bigger because we are adding the standard deviations of each parameter]
...
NN_fit <- nnetar(ts_act_power_daily_train,
                 p=1,
<<<<<<< HEAD:Lessons/M10_AdvancedForecastingModels_SP.Rmd
                 P=0, #seasonal is 0 because it is represented in the fourier terms 
=======
                 P=0,
>>>>>>> e60e521a3083942dec9fa90585d5c0036c5732cf:Lessons/M10_AdvancedForecastingModels.Rmd
...
-   Try including just one seasonal period, i.e., 7.
    Since we just have a few year, we may not have enough to model the influence of season of the year on demand.
